...
public class LoginVistaController implements Initializable {
    ...
    private void comprobarChbox(ActionEvent event) {
        ...
        if (evt.equals(cboxLocal)) {
            Variables.setOpcionCheckBox(1);   //guardamos en la variable el valor seleccinado (dento del método Variables)
            ...
            Variables.setOpcionCheckBox(2);    //guardamos en la variable el valor seleccinado (dento del método Variables)
            cboxLocal.setSelected(false);    //deselecciomos el check pulsado de los restantes checkbox
            cboxFichero.setSelected(false);  //deselecciomos el check pulsado de los restantes checkbox
        } else if (evt.equals(cboxFichero)) {
            Variables.setOpcionCheckBox(3);  //guardamos en la variable el valor seleccinado (dento del método Variables)
            cboxRemoto.setSelected(false); //deselecciomos el check pulsado de los restantes checkbox
            cboxLocal.setSelected(false);  //deselecciomos el check pulsado de los restantes checkbox
        }
    }
    ...
    public boolean validarCampos() {
        if (txtUsuario.getText().length() == 0 || txtUsuario.getText().length() > 5) {
            MensajeFX.printTexto("El campo USUARIO debe tener un tamaño entre 1 a 5 carácteres", "WARNING", posicionX_Y());
            txtUsuario.requestFocus();
            return false;
        }

        if (txtPassword.getText().isEmpty() || txtPassword.getText().length() > 8) {
            MensajeFX.printTexto("El campo CLAVE debe tener un tamaño entre 1 a 8 carácteres", "WARNING", posicionX_Y());
            txtPassword.requestFocus();
            return false;
        }
        //comprobamos sino tiene seleccionado un checkBox
        if (!seleccionCheckBox()) {
            MensajeFX.printTexto("Seleccione un sistema de conexión a la Base de Datos", "WARNING", posicionX_Y());
            return false;
        }
    ...
    public void validarAcceso() {
        try {
            ...
            if (respuesta.equals("OK")) {
                MensajeFX.printTexto("Acceso correcto", "INFO", posicionX_Y());
                this.cargarPrincipalVista(); //iniciamos correctamente
            } else {
                MensajeFX.printTexto("Datos de acceso incorrectos", "ERROR", posicionX_Y());
            }
